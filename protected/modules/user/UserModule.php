...
class UserModule extends CWebModule
{
		...
		if(Yii::app()->user->isGuest)
			...
			if (!isset(self::$_admin)) {				    
				if(self::user()->superuser)
                    ...
                    if(Yii::app()->controller->id=='personas'){                        
	 ...
	 * Send mail method
	 */
	public static function sendMail($email,$subject,$messageText){
        ...
        $message->setBody($messageText, 'text/html',''.Yii::app()->charset);
        $message->subject = $subject; 
        $message->addTo($email);
        $message->from = Yii::app()->params['adminEmail'];
        Yii::app()->mail->send($message); 
	    
	    return true;//mail($email,'=?UTF-8?B?'.base64_encode($subject).'?=',$message,$headers);
	}
	...
	public static function user($id=0,$clearCache=false) {
		...
		if ($id) {
            if (!isset(self::$_users[$id])||$clearCache){
                //RCPself::$_users[$id] = User::model()->with(array('profile'))->findbyPk($id);
