...
class UserRecoveryForm extends CFormModel {
	public $login_or_email, $user_id,$user_name;
	...
	{
		return array(
			'login_or_email'=>UserModule::t("username"),
		);
	}
	...
	public function checkexists($attribute,$params) {
		if(!$this->hasErrors())  // we only want to authenticate when no input errors
		{
			...
			//Debemos validar si el username es entero para catalogarlo como alumno
            $rut=str_replace(".","",trim($this->login_or_email));//Eliminando los puntos del username
            $rut=str_replace("-","",$rut);//Eliminando guion en caso de ser RUT
            if(ctype_digit(substr ($rut, 0, -1))){
            	$rut=substr ($rut, 0, -1);//En caso de ser numero, debemos eliminar el último caracter(digito verificador)
			}
			
			$user=User::model()->findByAttributes(array('username'=>$rut));
			if ($user){
