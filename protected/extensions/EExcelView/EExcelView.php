<?php
    ...
    * @version 0.3
    */  
    class EExcelView extends BsGridView
    {
        //Document properties
        public $creator = '';
        public $title = null;
        ...
        public $libPath = 'ext.phpexcel.PHPExcel'; //the path to the PHP excel lib
        ...
        //buttons config
        public $exportButtonsCSS = 'summary text-right';
        ...
        public function renderHeader()
        {
            ...
            foreach($this->columns as $column)
            {
                ...
                if($column instanceof CButtonColumn)
                    $head = utf8_encode($column->header);
                elseif($column->header===null && $column->name!==null)
                {
                    if($column->grid->dataProvider instanceof CActiveDataProvider)
                        $head = utf8_encode($column->grid->dataProvider->model->getAttributeLabel($column->name));
                    else
                        $head = utf8_encode($column->name);
                } else
                    $head =utf8_encode(trim($column->header)!=='' ? $column->header : $column->grid->blankDisplay);
        ...
        public function renderBody()
        {
            ...
            {           
                //$data=$this->dataProvider->getData();
                for($row=0;$row<$n;++$row){                 
                    $this->renderRow($row,$data,$columnsData);
                }
        ...
        public function renderRow($row,$data,$columnsData)
        {
            //$data=$this->dataProvider->getData();         

            $a=0;
            
            foreach($columnsData as $n=>$column)
            {
                if($column instanceof CLinkColumn)
                {
                    if($column->labelExpression!==null)
                        $value=utf8_encode($column->evaluateExpression($column->labelExpression,array('data'=>$data[$row],'row'=>$row)));
                    else
                        $value=utf8_encode($column->label);
                } elseif($column instanceof CButtonColumn)
                    $value = ""; //Dont know what to do with buttons
                elseif($column->value!==null){
                        $value=utf8_encode($this->evaluateExpression($column->value,array('data'=>$data[$row])));    
                }                   
                elseif($column->name!==null) { 
                    //$value=$data[$row][$column->name];
                    $value= utf8_encode(CHtml::value($data[$row], $column->name));
        ...
        public function run()
        {
            if($this->grid_mode == 'export')
            {
                ...
                else //output to browser
                {
                    ...
                    $this->cleanOutput();header('Cache-Control: must-revalidate, post-check=0, pre-check=0');
                    header('Pragma: public');
                    header('Content-type: '.$this->mimeTypes[$this->exportType]['Content-type']);
					//header('Content-type: '.$this->mimeTypes[$this->exportType]['Content-type'].'; charset='.Yii::app()->charset);
                    header('Content-Disposition: attachment; filename="'.$this->filename.'.'.$this->mimeTypes[$this->exportType]['extension'].'"');
                    header('Cache-Control: max-age=0'); 
                    if($this->mimeTypes[$this->exportType]['Content-type']=='application/csv'){
        ...
        public function renderExportButtons()
        {
            foreach($this->exportButtons as $key=>$button)
            {
                ...
                if (key_exists('query', $url))
                    $content[] = CHtml::link('<span class="glyphicon glyphicon-download-alt"> Exportar</span>', '?'.$url['query'].'&exportType='.$type.'&'.$this->grid_mode_var.'=export',array('class'=>'btn'));          
                else
                    $content[] = CHtml::link('<span class="glyphicon glyphicon-download-alt"> Exportar</span>', '?exportType='.$type.'&'.$this->grid_mode_var.'=export',array('class'=>'btn'));             
            }
            if($content)
