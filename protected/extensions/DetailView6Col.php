 ...
 *
 */
class DetailView6Col extends CWidget
{
	private $_formatter;
	...
	public $itemTemplateCenter="<th>{label}</th><td>{value}</td>\n";
	public $itemTemplateRight="<th>{label}</th><td>{value}</td></tr>\n";
	public $itemTemplateOneRow="<tr class=\"{class}\"><th>{label}</th><td colspan=5>{value}</td></tr>\n";
	public $itemTemplateHeader="<tr class=\"{class}\"><th colspan=6>{label}</th></tr>\n";
	/**
	 ...
	 */
	public $htmlOptions=array();//array('class'=>'detail-view');
	/**
	...
	public function run()
	{
		...
		foreach($this->attributes as $attribute)
		{
			...
			if( ($open_row == true && $open_center_row==true) && is_array($attribute) && (isset($attribute['header']) || isset($attribute['oneRow'])) )
				$i++;
			...
			if(is_array($attribute) && isset($attribute['header'])) {
				...
				if($open_center_row==true){
					echo strtr($this->itemTemplateCenter,$tr_empty); 
					$open_center_row=false;
				}
				if($open_row==true){
				...
				if(isset($attribute['oneRow']) && $attribute['oneRow'] === true) {
					...
					if(isset($attribute['newRow']) && $attribute['newRow'] === true) {
						...
						$open_row = true;
						$open_center_row=true;
					}else{
						if($open_row == true || $open_center_row==true) {
							if($open_center_row==true){
								echo strtr($this->itemTemplateCenter,$tr); 
								$open_center_row=false;
							}else if($open_row==true){
